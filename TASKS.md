...
| BF-8 | ✅ | market.model.ts — 4 merged-column lines breaking Drizzle types | `backend/src/models/market.model.ts` | L25,43,66,91 |
...
| 2A-3 | ✅ | Update fix_schema.js for coin_memory migration | `backend/fix_schema.js` |
| 2A-4 | ✅ | Create coin-memory.service.ts (saveMemory + getRecentMemory) | `backend/src/services/coin-memory.service.ts` |

### Phase 2B: Deep Article Generation (Data Augmentation)
| ID | Status | Task | Files |
|----|--------|------|-------|
| 2B-1 | ✅ | Create data-augmenter service (gather market + onchain + tavily + memory) | `backend/src/services/ai/data-augmenter.ts` |
...
| 2B-4 | ✅ | Create article-generator service (orchestrates augmenter + DeepSeek + publisher) | `backend/src/services/ai/article-generator.ts` |
...
| 2C-2 | ✅ | Create article-publisher service (save to coin_news + coin_memory + radar) | `backend/src/services/ai/article-publisher.ts` |
...
| 3A-2 | ✅ | Add `mode` parameter to `buildChatMessages` in PromptFactory + connect in openai.service.ts | `backend/src/services/ai/prompt-factory.ts`, `openai.service.ts` |

### Phase 3B: CONTEXT AI Mode (Event-Triggered)
| ID | Status | Task | Files |
|----|--------|------|-------|
| 3B-1 | ✅ | Update chat controller — Context Mode: fetch coin_memory + requireAuth route | `backend/src/controllers/chat.controller.ts` |
| 3B-2 | ✅ | Split chat routes — /stream (optionalAuth) + /stream/context (authMiddleware) | `backend/src/routes/chat.routes.ts` |

### Phase 3C: Guest Limitation System
| ID | Status | Task | Files |
|----|--------|------|-------|
| 3C-1 | ✅ | Create guest-limit middleware (Redis-based, max 3 prompts per guest IP/24h) | `backend/src/middleware/guest-limit.middleware.ts` |
...
| ID | Status | Task | Files |
|----|--------|------|-------|
| 3D-1 | ✅ | Create GET `/chat/context/:articleId/:articleType` endpoint | `backend/src/controllers/chat.controller.ts`, `backend/src/routes/chat.routes.ts` |

### Phase 3E: Terms & Conditions / Disclaimer
| ID | Status | Task | Files |
|----|--------|------|-------|
| 3E-1 | ✅ | Create POST `/chat/disclaimer-accept` + GET `/chat/disclaimer-status` endpoints | `backend/src/controllers/chat.controller.ts`, `backend/src/routes/chat.routes.ts` |
